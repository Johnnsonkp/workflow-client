...
const ShiftCard: React.FC<Props>  = ({obj, todaysDate, toggleDelete, setToggleDelete, index}) => {
    ...
    return (
        obj.task && obj.task?.length > 0? obj.task?.map((task: any) => (
            <div key={index + task.order + task.title} 
                className={`relative cursor-pointer border ${task ? 'border-gray-300' : 'border-gray-200'} flex-col mt-2 text-[12px] h-32 w-32 rounded-md  p-1 
